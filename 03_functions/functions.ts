...
// ==============================
function addTyped(num1: number, num2: number): number {
  return num1 + num2;
}

let myAddTyped = function (num1: number, num2: number): number {
  return num1 + num2;
};

// Writing the function type (full type of the function)
let myFullType: (num1: number, num2: number) => number = function (
  num1: number,
  num2: number
): number {
  return num1 + num2;
...
let incrementFunc: (baseValue: number, increment: number) => number = function (
  x: number,
  y: number
): number {
  return x + y;
...
let myLessTypedAdd = function (x: number, y: number): number {
  return x + y;
};
...
// Default values
function buildName2(firstName: string, lastName = "Bond"): string {
...
// Now TypeScript knows that createCardPicker expects to be called
// on a Deck object. That means that this is of type Deck now, not any,
// so --noImplicitThis will not cause any errors.

// *** this parameters in callbacks ***

// You can also run into errors with this in callbacks, when you pass
// functions to a library that will later call them.
